...
const StatusReport = () => {
  ...
  const getAllTickets = async () => {
    try {
      ...
      const numSold = parseInt(response.data.slice(response.data.indexOf(":") + 1).trim())
  ...
  const getMovies = async (location) => {
    ...
    try {
      ...
      const theaterIds = locationTheaters.flatMap((theater) => theater.movieIds)
      const playingMovies = allMovies.filter((movie) => theaterIds.includes(movie.id))

      playingMovies.sort((a, b) => (a.title > b.title ? 1 : -1))
      setPlayingMovies(playingMovies)
      setMovieCache((prevCache) => ({ ...prevCache, [location]: playingMovies }))
    } catch (err) {
      alert(err.message)
    }
  }
  ...
  const getTicketsByLocation = async (location) => {
    try {
      const response = await axios.get(`/api/tickets/location/total?location=${location}`)
  ...
  const getTickets = async (location) => {
    ...
    try {
      const response = await axios.get(`/api/tickets/location?location=${location}`);
      setTickets(response.data)
      setTicketCache((prevCache) => ({ ...prevCache, [location]: response.data }));
    } catch (err) {
      alert(err.message)
    }
  }
  ...
  const getTicketsByMovie = (movieId) => {
    return tickets.filter((ticket) => ticket.movieId === movieId).reduce((acc, ticket) => acc + ticket.numberPurchased, 0)
  }
      ...
      {isLoggedIn && isAdmin ? (
        <div className="status_report_container_sr">
          <div className="status_report_div">
            <Button className="status_report_button_sr" variant="danger" onClick={goBack}>
              Return
            </Button>
