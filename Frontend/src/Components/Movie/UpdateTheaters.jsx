...
const UpdateTheaters = () => {
  ...
  const getTheaters = async () => {
    try {
      const locationTheaters = await axios.get(
        `/api/theaters/get/by/location?location=${location}`
      );
      const sortedTheaters = locationTheaters.data.sort(
  ...
  const updateTheater = async (theaterLoc, theaterRoomNum, movieId, time) => {
    ...
    try {
      const updated = await axios.patch(
        `/api/theaters/change/movie?location=${theaterLoc}&roomNumber=${theaterRoomNum}&movieId=${movieId}&time=${time}`
      );
      alert(updated.data);
    } catch (err) {
      alert(err.message);
    ...
    <div className="update_theater_top_div_s">
      {isLoggedIn && isAdmin ? (
        <div className="update_theater_container_s">
          ...
          <Button className="update_theater_button_s" onClick={() => nav(-1)}>
            return
          </Button>
